...
def get_spreadsheet(searchKey):
    ...
    for i in range(1, len(records)):        # sheetの行数分だけ繰り返す
        if records[i][0] == searchKey:     # １列目がTerminalIDと一致するま>で
            ...
            gdist=records[i][5]
            guser=records[i][6]
            print(gdate, gtime, gjump, gdur, gcal, gdist, guser)
    return gdate, gtime, gjump, gdur, gcal, gdist, guser
...
while True:
    ...
    for dev in devices:
        ...
        for (adtype, desc, val) in dev.getScanData():
            #print("  %s = %s" % (desc, val))
            if desc == 'Short Local Name' and val[0:10] == 'ROHMMedal2': #_0040_01': #Short Local Name = ROHMMedal2_0040_01.00
                isRohmMedal = True
                print(val)
                if val[12:15] == "0040":
                    cur_user = 'K'
            ...
            if isRohmMedal and desc == 'Manufacturer':
                ...
                if SEQ in [255,0,1] and step_cnt == 0:
                    ...
                    if last_cnt!=0:
                        ...
                        sheet.append([last_time.strftime('%Y/%m/%d'), last_time.strftime('%H:%M:%S'), 
                            last_cnt, last_dur, last_cal, last_mv, cur_user])
                else:
                ...
                if start_seq >= 1:
                    ...
                    if dur_seconds > 3600:
                        dur_text = str(round(dur_seconds / 3600,1)) + "h."
                    elif dur_seconds > 60:
                        dur_text = str(round(dur_seconds / 60)) + "min"
                    else:
                        dur_text = str(dur_seconds) + "s."
                    jump_text= dur_text+"!"+str(cur_cnt)+"cnt!"+str(cur_cal)+"cal."
                    ...
                    if cur_cnt > 10:
                        ...
                        if cur_cnt%100 == 0:
                            ...
                            total_cal = totals[4]
                            total_food= round(int(total_cal)/200)
                            total_mv  = totals[5]
                            ...
                            mv_text   = cur_user + str(cur_mv)+"Km to "+str(round(cur_acc_dist-int(total_mv)))+"Km"
                            #c.itemconfigure(c3, text=mv_text)
                            lcd.set_cursor_position(0,1)
                            lcd.write(mv_text)
                            total_text = "Total "+str(total_hour)+"spent,"+total_jump+"jump "+total_cal+"Cal."
                            ...
                            lcd.set_cursor_position(0,3)
                            lcd.write(total_text)
